...
neonanobot is a lightweight, open-source AI agent framework written in Python with a React/TypeScript WebUI. It centers around a small agent loop that receives messages from chat channels, invokes an LLM provider, executes tools, and manages session memory.
...
Messages flow through an async `MessageBus` (`neonanobot/bus/queue.py`) that decouples chat channels from the agent core:

1. **Channels** (`neonanobot/channels/`) receive messages from external platforms and publish `InboundMessage` events to the bus.
2. **`AgentLoop`** (`neonanobot/agent/loop.py`) consumes inbound messages, builds context, and coordinates the turn.
3. **`AgentRunner`** (`neonanobot/agent/runner.py`) handles the actual LLM conversation loop: send messages to the provider, receive tool calls, execute tools, and stream responses.
...
- **LLM Providers** (`neonanobot/providers/`): Provider implementations (Anthropic, OpenAI-compatible, etc.) built on a common base (`base.py`). `factory.py` and `registry.py` handle instantiation and model discovery.
...
- **Tools** (`neonanobot/agent/tools/`): Agent capabilities exposed to the LLM: filesystem (read/write/edit/list), shell execution (with sandbox backends), web search/fetch, cron, notebook editing, subagent spawning, long-running tasks / sustained goals (`long_task.py`), and self-modification. Tools are auto-discovered via `pkgutil` scan + entry-point plugins.
...
- **WebUI** (`webui/`): Vite-based React SPA that talks to the gateway over a WebSocket multiplex protocol. The dev server proxies `/api`, `/webui`, and `/auth` to the gateway; app WebSocket connections go directly to the gateway.
...
- **CLI**: `neonanobot/cli/commands.py`
- **Python SDK**: `neonanobot/neonanobot.py`
...
- Config schema: `neonanobot/config/schema.py`
- Provider base / new provider template: `neonanobot/providers/base.py`
- Channel base / new channel template: `neonanobot/channels/base.py`
- Tool registry: `neonanobot/agent/tools/registry.py`
- WebUI dev proxy config: `webui/vite.config.ts`
- Tests mirror the `neonanobot/` package structure.
