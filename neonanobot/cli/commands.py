...
# Remove default handler and re-add with unified neonanobot format
logger.remove()
...
from neonanobot import __logo__, __version__
...
from neonanobot.cli.stream import StreamRenderer, ThinkingSpinner
from neonanobot.config.paths import get_workspace_path, is_default_workspace
from neonanobot.config.schema import Config
from neonanobot.utils.evaluator import evaluate_response
from neonanobot.utils.helpers import sync_workspace_templates
from neonanobot.utils.restart import (
...
app = typer.Typer(
    ...
    help=f"{__logo__} neonanobot - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from neonanobot.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    if show_header:
        console.print()
        console.print(f"[cyan]{__logo__} neonanobot[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} neonanobot[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} neonanobot v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """neonanobot - Personal AI Assistant."""
    pass
...
def onboard(
    ...
    """Initialize neonanobot configuration and workspace."""
    from neonanobot.config.loader import get_config_path, load_config, save_config, set_config_path
    from neonanobot.config.schema import Config
    ...
    if wizard:
        from neonanobot.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'neonanobot onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'neonanobot agent -m "Hello!"'
    gateway_cmd = "neonanobot gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} neonanobot is ready!")
    console.print("\nNext steps:")
    if wizard:
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from neonanobot.channels.registry import discover_all
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from neonanobot.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show neonanobot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'neonanobot-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger

    from neonanobot.api.server import create_app
    from neonanobot.bus.queue import MessageBus
    from neonanobot.session.manager import SessionManager

    if verbose:
        logger.enable("neonanobot")
    else:
        logger.disable("neonanobot")
...
def _load_or_create_desktop_config(config: str | None, workspace: str | None) -> Config:
    ...
    from neonanobot.config.loader import (
        get_config_path,
        load_config,
    ...
    from neonanobot.config.schema import Config as NanobotConfig
...
def desktop_gateway(
    ...
    """Start the private local gateway used by neonanobot Desktop."""
...
def _run_gateway(
    ...
    *,
    ...
    from neonanobot.agent.tools.cron import CronTool
    from neonanobot.agent.tools.message import MessageTool
    from neonanobot.bus.queue import MessageBus
    from neonanobot.channels.manager import ChannelManager
    from neonanobot.channels.websocket import publish_runtime_model_update
    from neonanobot.cron.service import CronService
    from neonanobot.cron.types import CronJob
    from neonanobot.providers.factory import build_provider_snapshot, load_provider_snapshot
    from neonanobot.session.manager import SessionManager

    port = port if port is not None else config.gateway.port

    console.print(f"{__logo__} Starting neonanobot gateway version {__version__} on port {port}...")
    ...
    from neonanobot.agent.loop import UNIFIED_SESSION_KEY
    from neonanobot.bus.events import OutboundMessage
    ...
    from neonanobot.cron.types import CronJob, CronPayload, CronSchedule
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show neonanobot runtime logs during chat"),
):
...
def channels_status(
    ...
    """Show channel status."""
    from neonanobot.channels.registry import discover_all
    from neonanobot.config.loader import load_config, set_config_path
...
def channels_login(
    channel_name: str = typer.Argument(..., help="Channel name (e.g. feishu)"),
...
def plugins_list():
    ...
    from neonanobot.channels.registry import discover_all, discover_channel_names
    from neonanobot.config.loader import load_config
...
@app.command()
def status():
    """Show neonanobot status."""
    from neonanobot.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} neonanobot Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from neonanobot.providers.registry import PROVIDERS
...
def _login_github_copilot() -> None:
    try:
        from neonanobot.providers.github_copilot_provider import login_github_copilot
