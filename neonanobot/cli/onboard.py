"""Interactive onboarding questionnaire for neonanobot."""
...
from neonanobot.cli.models import (
    format_token_count,
    get_model_context_limit,
    get_model_suggestions,
)
from neonanobot.config.loader import get_config_path, load_config
from neonanobot.config.schema import Config, ModelPresetConfig
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]neonanobot[{__version__}][/bold cyan]")
    )
    console.print()
...
def _handle_fallback_models_field(
    ...
    from neonanobot.config.schema import InlineFallbackConfig
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from neonanobot.config.schema to get
    ...
    from neonanobot.config.schema import AgentDefaults
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"neonanobot.channels.{name}")
