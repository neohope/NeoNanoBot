...
from neonanobot.apps.protocol import app_manifest, compact_dict
from neonanobot.config.paths import get_runtime_subdir
from neonanobot.security.workspace_policy import is_path_within

CLI_ANYTHING_REGISTRY_URL = "https://neohope.github.io/CLI-Anything/registry.json"
CLI_ANYTHING_PUBLIC_REGISTRY_URL = "https://neohope.github.io/CLI-Anything/public_registry.json"
CLI_ANYTHING_RAW_BASE = "https://raw.githubusercontent.com/neohope/CLI-Anything/main"
NANOBOT_EXTENSION_REGISTRY_URL = "https://raw.githubusercontent.com/Re-bin/neonanobot-extension/main/registry.json"
NANOBOT_EXTENSION_RAW_BASE = "https://raw.githubusercontent.com/Re-bin/neonanobot-extension/main"
_CATALOG_SOURCES = (
...
_BRAND_DOMAINS: dict[str, tuple[str, str]] = {
    ...
    "macrocli": ("github.com/neohope/CLI-Anything/tree/main/macrocli", "#24292F"),
...
class CliAppManager:
    ...
    def _manifest_source(self, app: dict[str, Any]) -> str:
        ...
        if source == "extensions":
            return "neonanobot-extension"
        return f"cli-anything:{source}"

    def _trust_registry(self, app: dict[str, Any]) -> str:
        return "neonanobot-extension" if str(app.get("_source") or "") == "extensions" else "cli-anything"
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from neonanobot."
        return f"""---
...
Use this skill when the user asks neonanobot to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_nanobot_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- neonanobot-cli-app-note -->"
        if marker in content:
            return content
        name = str(app.get("name") or "unknown")
        note = f"""{marker}
## neonanobot execution
...
"""
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy != "bundled":
            ...
            if still_managed or (not managed_entry_path and still_available):
                ...
                message = (
                    ...
                    "so neonanobot kept it installed."
                )
                return self.payload() | {
                    "last_action": {
                        "ok": False,
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from neonanobot. {entry_point} "
                "is still available because it is managed outside neonanobot."
            )
