"""WebSocket server channel: neonanobot acts as a WebSocket server and serves connected clients."""
...
from neonanobot.bus.events import OUTBOUND_META_AGENT_UI, OutboundMessage
from neonanobot.bus.queue import MessageBus
from neonanobot.channels.base import BaseChannel
from neonanobot.command.builtin import builtin_command_palette
from neonanobot.config.paths import get_media_dir, get_workspace_path
from neonanobot.config.schema import Base
from neonanobot.session.goal_state import goal_state_ws_blob
from neonanobot.session.webui_turns import websocket_turn_wall_started_at
from neonanobot.utils.media_decode import (
    FileSizeExceeded,
...
from neonanobot.webui.thread_disk import delete_webui_thread
from neonanobot.webui.transcript import (
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      neonanobot and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-neonanobot-Auth: <secret>``.
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-neonanobot-Auth") or headers.get("x-neonanobot-auth")
    if not header_token:
        return False
...
class WebSocketChannel(BaseChannel):
    ...
    async def start(self) -> None:
        from neonanobot.utils.logging_bridge import redirect_lib_logging
