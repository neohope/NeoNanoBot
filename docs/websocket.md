...
neonanobot can act as a WebSocket server, allowing external clients (web apps, CLIs, scripts) to interact with the agent in real time via persistent connections.
...
```text
WebSocket server listening on ws://127.0.0.1:4321/
```
...
```bash
# Using websocat
websocat ws://127.0.0.1:4321/?client_id=alice

# Using Python
import asyncio, json, websockets

async def main():
    async with websockets.connect("ws://127.0.0.1:4321/?client_id=alice") as ws:
        ready = json.loads(await ws.recv())
        print(ready)  # {"event": "ready", "chat_id": "...", "client_id": "alice"}
        await ws.send(json.dumps({"content": "Hello neonanobot!"}))
...
1. Client sends `GET {tokenIssuePath}` with `Authorization: Bearer {tokenIssueSecret}` (or `X-neonanobot-Auth` header).
...
```bash
# 1. Obtain a token
curl -H "Authorization: Bearer your-secret-here" http://127.0.0.1:4321/auth/token

# 2. Connect using the token
websocat "ws://127.0.0.1:4321/ws?client_id=alice&token=nbwt_aBcDeFg..."
```
...
`chat_id` is a *capability*: anyone holding a valid WebSocket auth credential and the chat_id can attach to that conversation and see its output. This is safe for neonanobot's local, single-user model. Multi-tenant deployments should namespace chat_ids per user (or introduce a per-tenant auth gate) — neonanobot does not do this today.
...
- An HTTP file server serving the neonanobot media directory
...
Clients connect to `ws://127.0.0.1:4321/chat/ws?client_id=...`. Trailing slashes are normalized, so `/chat/ws/` works the same.
