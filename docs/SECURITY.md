...
If you discover a security vulnerability in neonanobot, please report it by:
...
**Recommendations:**
- Store API keys in `~/.neonanobot/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.neonanobot/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run neonanobot with a dedicated user account
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash neonanobot
   sudo -u neonanobot neonanobot gateway
   ```
...
4. **Enable Logging**
   ...
   tail -f ~/.neonanobot/logs/neonanobot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.neonanobot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.neonanobot/logs/neonanobot.log
   ```
