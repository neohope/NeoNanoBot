...
neonanobot can expose a minimal OpenAI-compatible endpoint for local integrations:

```bash
pip install "neonanobot-ai[api]"
neonanobot serve
```
...
- API requests run in the synthetic `api` channel, so the `message` tool does **not** automatically deliver to Feishu/etc. To proactively send to another chat, call `message` with an explicit `channel` and `chat_id` for an enabled channel.
...
If `channel` points to a channel that is not enabled in your config, neonanobot will queue the outbound event but no platform delivery will occur.
