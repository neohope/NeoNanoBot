...
Use neonanobot as a library — no CLI, no gateway, just Python.
...
from neonanobot import neonanobot


async def main() -> None:
    bot = neonanobot.from_config()
...
`neonanobot.from_config()` reuses your normal `~/.neonanobot/config.json`, so the SDK follows the same provider, model, tools, and workspace defaults as the CLI unless you override them.
...
bot = neonanobot.from_config(
    config_path="~/.neonanobot/config.json",
...
Hooks let you inspect tool calls, streaming, and iteration state without modifying neonanobot internals:

```python
from neonanobot.agent import AgentHook, AgentHookContext
...
### `neonanobot.from_config(config_path=None, *, workspace=None)`

Create a `neonanobot` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.neonanobot/config.json`. |
...
```python
from neonanobot.agent import AgentHook
...
async def main() -> None:
    bot = neonanobot.from_config(workspace="/my/project")
