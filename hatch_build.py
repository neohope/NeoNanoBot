"""Hatch build hook that bundles the webui (Vite) into neonanobot/web/dist.
...
class WebUIBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:  # noqa: D401
        ...
        dist_dir = root / "neonanobot" / "web" / "dist"
        ...
        if not package_json.is_file():
            self.app.display_info(
                "[webui-build] no webui/ source tree, assuming prebuilt neonanobot/web/dist/"
            )
            return
