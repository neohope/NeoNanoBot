...
The browser front-end for the neonanobot gateway. It is built with Vite + React 18 +
...
If you installed neonanobot via `pip install neonanobot-ai`, the WebUI is **already bundled** in the wheel. Enable the WebSocket channel in `~/.neonanobot/config.json` and run `neonanobot gateway` — see the root [`README.md`](../README.md#-webui) for the 3-step setup. You do **not** need anything in this directory.
...
neonanobot/web/dist/      build output served by the gateway
```

## Develop the WebUI (Vite HMR)

### 1. Install neonanobot from source
...
By default the dev server proxies `/api` and `/webui` to `http://127.0.0.1:4321`. The app WebSocket connects directly to the gateway rather than through the dev server.
...
To use the WebUI from another device on the same network, set `host` to `"0.0.0.0"` and configure a `token` or `tokenIssueSecret` in `~/.neonanobot/config.json`:
...
Then open `http://<your-ip>:4321` on the other device. The WebUI will show an authentication form where you enter the secret. It is saved in your browser so you only need to enter it once.
...
```bash
cd webui
bun run build          # writes to ../neonanobot/web/dist
```
